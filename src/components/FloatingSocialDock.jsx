...
const SOCIAL_DOCK_ITEMS = [
  ...
  },
  {
    id: 'instagram',
    title: 'Instagram',
    href: 'https://instagram.com/fazzil_firoz',
...
export function FloatingSocialDock() {
  ...
  // Calculate center shift offset for mobile / desktop
  const rightMargin = screenWidth <= 768 ? 20 : 28
  const iconSize = 56
  const centerShift = -(screenWidth / 2 - iconSize / 2 - rightMargin)

  return (
    <div className="social-dock-wrapper" aria-label="Social links dock">
      {/* ── STAGE 1: Single WhatsApp Floating Button at Bottom-Right (When not at bottom) ── */}
      ...
      {/* ── STAGE 2: Wheel to Middle + 5-Icon 3D Orbital Dock Burst (When at bottom) ── */}
      <AnimatePresence>
        {isAtBottom && (
          <motion.div
          ...
          >
            <div className="bottom-dock-center-wrapper">
              {SOCIAL_DOCK_ITEMS.map((item, index) => {
                // Calculate horizontal offset for 5 icons centered in a row
                const total = SOCIAL_DOCK_ITEMS.length
                const offsetStep = screenWidth <= 480 ? 56 : 68
